...
export default function CommentList({ postId }) {
  ...
  const renderComment = (comment, isReply = false) => {
    ...
    return (
      <div
        key={comment.id}
        className={`${
          isReply ? "ml-8 mt-6 border-l-2 border-gray-200 dark:border-gray-700 pl-4" : "mb-8"
        } bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow`}
      >
        <div className="flex items-start">
          <div className="h-10 w-10 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-600 dark:text-gray-300 text-lg font-medium flex-shrink-0">
            {comment.authorName?.charAt(0).toUpperCase() || "U"}
          </div>
          <div className="ml-4 flex-1">
            <div className="flex items-center justify-between">
  ...
  if (isLoading) {
    return (
      <div className="text-center py-12 text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900 rounded-lg">
        Loading comments...
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="text-center py-12 text-red-600 dark:text-red-400 bg-gray-50 dark:bg-gray-900 rounded-lg">
        Error loading comments
      </div>
    );
  }
