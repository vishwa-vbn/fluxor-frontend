...
const MediaView = ({
  ...
  activeTab = "imagekit",
  onTabChange = () => {},
  isCloudinary = false, // Flag to disable folder-related features for Cloudinary
}) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedAsset, setSelectedAsset] = useState(null);
        ...
        <main className="flex-1 max-w-[100%] w-full mx-auto px-6 py-3 space-y-8">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex space-x-2">
              ...
              {!isCloudinary && ( // Hide folder creation for Cloudinary
                <Button
                  onClick={() => {
                    const folderName = prompt("Enter folder name:");
                    if (folderName) onCreateFolder(folderName);
          ...
          <Tabs value={activeTab} onValueChange={onTabChange}>
            ...
            <TabsContent value={activeTab}>
              {loading ? (
                ...
                <div>
                  {!isCloudinary && filteredFolders.length > 0 && ( // Hide folders for Cloudinary
                    <div className="mb-6">
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                        {filteredFolders.map((folder) => (
                          <div
                  ...
                  {mediaAssets.length > 0 && (
                    <div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                        {mediaAssets.map((asset) => (
                          <div
                            key={asset.fileId || asset.assetId}
                            className="relative rounded-[5px] overflow-hidden bg-white dark:bg-gray-800 cursor-pointer"
                          >
                            <div className="relative aspect-square">
                              {asset.fileType === "non-image" && asset.url ? (
                            ...
                            <div className="absolute top-2 right-2 flex space-x-1">
                              ...
                              <Button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  onDeleteFile(asset.fileId || asset.assetId);
                                }}
                                className="p-1 bg-white dark:bg-gray-800 rounded-[5px] text-red-500 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-700 shadow-sm"
                              >
                                <Trash2 size={12} />
