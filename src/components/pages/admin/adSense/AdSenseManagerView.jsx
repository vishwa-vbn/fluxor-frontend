...
const AdSenseManagerView = ({
  ...
  const [previewAd, setPreviewAd] = useState(null);
  const [activeTab, setActiveTab] = useState("configuration");
  const [formData, setFormData] = useState(adSenseConstants.normalizeAdData({}));
  const [errors, setErrors] = useState({});

  const defaultAd = useMemo(() => adSenseConstants.defaultAd, []);

  useEffect(() => {
    if (activeTab === "create" && editingAd) {
      setFormData(adSenseConstants.normalizeAdData(editingAd));
    }
  }, [editingAd, activeTab]);
  ...
  const handleTabChange = (newTab) => {
    ...
    if (newTab !== "create") {
      setEditingAd(null);
      setFormData(adSenseConstants.normalizeAdData({}));
    } else {
      setFormData(
        editingAd
          ? adSenseConstants.normalizeAdData(editingAd)
          : adSenseConstants.normalizeAdData(defaultAd)
      );
    }
  };
          ...
          {!loading && (
            ...
            >
              ...
              <TabsContent value="custom" className="mt-6">
                <CustomAds
                  adUnits={adUnits}
                  onAddCustomAd={handleAddCustomAd}
                  onEditAd={handleEditAd}
                  onDeleteAdUnit={onDeleteAdUnit}
              ...
              <TabsContent value="create" className="mt-6">
                <AdForm
                  ...
                  setErrors={setErrors}
                  editingAd={editingAd}
                  onSaveAd={onAddCustomAd}
                  onUpdateAd={onUpdateCustomAd}
                  defaultAd={defaultAd}
                />
              </TabsContent>
            </Tabs>
