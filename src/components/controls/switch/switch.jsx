...
const Switch = ({
  ...
  disabled,
  label,
  activeColor = 'bg-blue-600 dark:bg-blue-500', // Default active color (blue)
  inactiveColor = 'bg-gray-200 dark:bg-gray-600', // Default inactive color
  focusRingColor = 'ring-blue-500 dark:ring-blue-400', // Default focus ring color
}) => {
  const handleChange = (event) => {
    if (!disabled) {
      console.log(`Switch ${id} toggled to: ${event.target.checked}`);
    ...
    <div className="relative inline-flex items-center">
      ...
      >
        <div
          className={`
            absolute top-0.5 left-0.5
            w-4 h-4 bg-white dark:bg-gray-100 rounded-full shadow-sm
            transform transition duration-200 ease-in-out
            ${checked ? 'translate-x-4' : 'translate-x-0'}
          `}
        />
